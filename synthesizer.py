...
class Synthesizer:
    ...
    def generate(self):
        ...
        # Split data to train and val before synthasizing
        return data
    
class SynthesizerDataset(Dataset):
    def __init__(self, synthesizer, num_samples=10000):
        self.synthesizer = synthesizer
        self.num_samples = num_samples  # how many samples you want to provide

    def __len__(self):
        return self.num_samples

    def __getitem__(self, idx):
        audio = self.synthesizer.generate()

        # Convert numpy arrays to PyTorch tensors
        return {
            "target": torch.tensor(audio["target"], dtype=torch.float32),
            "nearend": torch.tensor(audio["nearend"], dtype=torch.float32),
            "mic": torch.tensor(audio["mic"], dtype=torch.float32)
        }
...
if __name__ == '__main__':
    ...
    dataset = SynthesizerDataset(s, num_samples=10)

    dataloader = DataLoader(dataset, batch_size=8, shuffle=True, num_workers=4)

    for batch in dataloader:
        target = batch["target"]      # shape: (batch_size, seq_len)
        nearend = batch["nearend"]
        mic = batch["mic"]

        # You can now feed these into a model
        #print(target.shape, nearend.shape, mic.shape)
    

    #sf.write(r"./target.wav", audio['target'], s.nearend_datasets.sample_rate)
    #sf.write(r"./nearend.wav", audio['nearend'], s.nearend_datasets.sample_rate)
    #sf.write(r"./mic.wav", audio['mic'], s.nearend_datasets.sample_rate)
